...
if __name__ == '__main__':
    ...
    anchor_config_num = 5
    lr_list = np.random.uniform(low=1e-2, high=1e2, size=anchor_config_num)
    ...
    weight_decay_list = ['1e-06','1e-05', '0.0001','0.001','0.01', '1e-01', '1e00']
    ...
    wd_hp_list = [str(float(wd)) for wd in weight_decay_list]
    weight_decay_result_list_dict = {}
    for wd in wd_hp_list:
    ...
    for i in range(anchor_config_num):
        for wd in wd_hp_list:
            ...
            weight_decay_result_list_dict[wd].append(max_recall)
    ...
    f = open(logfilePath+'wdinfo'+'20'+'.json', 'w')
