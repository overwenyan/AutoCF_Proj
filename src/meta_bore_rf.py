...
class MetaClassifier:
    ...
    def data_init(self, hparams_list, performance_list):
        ...
        data_list = [{'opt': hparams_list[i][0], 'lr': hparams_list[i][1], 'embedding_dim': hparams_list[i][2] ,\
        ...
        self.targets = performance_list # which we want to maximize
    ...
    def get_next_hparams(self):
        ...
        if self.meta_mode == 'random':
            ...
            # self.hparams_list.append(hparams_next)
...
if __name__ == '__main__':
    ...
    storage_nums = 256
    trial_nums = 10 # we will draw a figure: trails vs performances
    # construct dataset 
    ...
    avaliable_device_ids = [5, 6] # 
    print("host {} avaliable_device_ids: {}".format(hostname, avaliable_device_ids))
    # avaliable_device_ids = avaliable_device_ids[:3]
    assigned_device_ids = [val for val in avaliable_device_ids for i in range(4)]  # final gpu-ids x 3
    ...
    with open('bore_rf_meta_log_{}.json'.format(arch_encoding), 'w') as json_file:
    ...
    for i in range(trial_nums):
        ...
        if args.data_type == 'implicit':# 装载
            ...
            args.num_users, args.num_items = num_users, num_items
            jobs = [arch_single, num_users, num_items, train_queue, valid_queue, test_queue, args, [lr_list[i], embedding_dim_list[i]], args.device, args.if_valid] 
            rmse_list1.append(list(get_single_model_performance(jobs)))


        hparams_list.append(curr_hparams)
        performance_list = rmse_list1
        print("hparams_list: {}".format(hparams_list))
        print("rmse_list1: {}".format(rmse_list1))
        
        # hparams_next = get_next_hparams(hparams_list, performance_list, mode='random')
        curr_hparams = hparams_next
    recall20_list = [p[0] for p in rmse_list1]
    plt.plot(recall20_list, list(range(trial_nums)) )
