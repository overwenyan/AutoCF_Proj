...
# parser.add_argument('--if_validaaafff', type=int, default=1, help='use validation set for tuning single architecture or not')
...
parser.add_argument('--anchor_num', type=int, default=5, help='size of anchor sample number')
...
if __name__ == '__main__':
    ...
    # for arch_count in range(anchor_config_num): # 相当于大循环的次数，一次循环随机一个模型，随机一组超参
    
    # 对给定的arch进行采样，可以选择遍历所有的arch
    ...
    # for lr in lr_hp_list:
        ...
        if hostname == 'fib':
            ...
            pass
        avaliable_device_ids = GPUtil.getAvailable(order = 'first', limit = 8, maxLoad = 0.5, maxMemory = 0.2, includeNan=False, excludeID=[], excludeUUID=[])
        print("avaliable_device_ids: {}".format(avaliable_device_ids))
        assigned_device_ids = avaliable_device_ids
        if len(avaliable_device_ids) == 0:
            print("Not enough avaliable_device_ids: {}!".format(avaliable_device_ids))
            sys.exit(1)
        # if run_one_model > 0:
        ...
        total_task_number = len(lr_hp_list)
        task_number = math.ceil(total_task_number / len(avaliable_device_ids)) 
        task_split = list(range(0, total_task_number, len(avaliable_device_ids)))
        task_split.append(total_task_number)
        ...
        for tasks in task_index: #下面对同一种架构进行不同超参的并行 每个task对应在不同GPU上的工作
            with mp.Pool(processes=len(tasks)) as p:
                print('lr: {} start!, tasks: {}'.format(lr, tasks))
                p.name = 'test'
                if args.data_type == 'implicit':# 装载任务
                    jobs = [[arch_single_list[arch_count], num_users, num_items, [], valid_queue, test_queue, train_queue_pair, args, [float(float(lr_hp_list[arch_count])), embedding_dim_list[arch_count]], assigned_device_ids[i % len(assigned_device_ids)], args.if_valid] for i in tasks] 
                    # 此处的tasks包括lr的各种结果，但是这一步的arch应该相同，同一组内的超参配置相同
                else:
                    jobs = [[arch_single_list[arch_count], num_users, num_items, train_queue, valid_queue, test_queue, args, [float(lr_hp_list[arch_count]), embedding_dim_list[arch_count]], assigned_device_ids[i % len(assigned_device_ids)], args.if_valid] for i in tasks]
                rmse_list1 = p.map(get_single_model_performance, jobs) # 返回的数值是tasks的合计
                print('arch_count: {}, len(rmse_list1): {}, rmse_list1: {}'.format(arch_count,len(rmse_list1), rmse_list1))
                print("task of {} ended".format(tasks))
                # print() # 每一个lr对应一组rmse_list（实际上也可能是recall list）
                # run_one_model += 1
                # lr_result_list_dict
                recall_list1 = [rmse1[0] for rmse1 in rmse_list1]
                lr_result_list_dict[lr].append(rmse_list1)
                p.close()
    ...
    f = open(logfilePath+'lrinfo'+'200'+'.json', 'w')
